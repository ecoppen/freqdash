...
from freqdash.exchange.binance import Exchange
...
class TestExchange(unittest.TestCase):
    def test_attributes(self):
        ...
        assert exchange.spot_api_url is None
        assert exchange.spot_trade_url is None
    ...
    def test_get_spot_price(self):
        exchange = Exchange()
        spot_price = exchange.get_spot_price(base="BTC", quote="USDT")
        assert spot_price == Decimal(-1.0)

    def test_get_spot_prices(self):
        exchange = Exchange()
        assert exchange.get_spot_prices() == []

    def test_get_spot_kline(self):
        exchange = Exchange()
        spot_kline = exchange.get_spot_kline(base="BTC", quote="USDT")
        assert spot_kline == []
